...
<head>
    <meta charset="UTF-8">
    <title><?php echo e($post['title']); ?> | Social Blog</title>
    <link rel="stylesheet" href="css/style.css">
    <style>
        .main-content { margin-left: 250px; padding: 40px; transition: 0.3s; }
        .full-post-view { display: block !important; padding: 40px !important; }
        .view-post-header { margin-bottom: 30px; border-bottom: 1px solid #eee; padding-bottom: 20px; }
        .view-post-image-container { width: 100%; display: flex; justify-content: center; background: #f8f9fa; border-radius: 12px; margin: 30px 0; overflow: hidden; }
        .view-post-image { max-width: 100%; max-height: 600px; object-fit: contain; }
        .comment-section { margin-top: 40px; display: block !important; }
        .comment-item { border-bottom: 1px solid #eee; padding: 20px 0; display: flex !important; }
    ...
    <div class="post-card full-post-view">
        ...
        <?php if (!empty($post['post_image'])): ?>
            <div class="view-post-image-container">
                <img src="uploads/post_images/<?php echo e($post['post_image']); ?>" class="view-post-image">
            </div>
        <?php endif; ?>
        ...
        <div style="margin-top: 30px; padding-top: 20px; border-top: 1px solid #eee;">
            <span style="font-size: 1.2em;">❤️ <strong><?php echo $like_count; ?></strong> Likes</span>
            <?php if ($user_id): ?>
    ...
    <div class="post-card full-post-view comment-section">
        ...
        <?php if ($user_id): ?>
            <form action="add_comment.php" method="POST">
                ...
                <textarea name="comment_text" required placeholder="Discussion..." style="width:100%; height:100px; padding:15px; border-radius:8px; border:1px solid #ddd;"></textarea>
                <button type="submit" class="button" style="margin-top:10px; background:#2ecc71;">Post Comment</button>
            </form>
        <?php endif; ?>
        <?php foreach ($comments as $comment): ?>
            <div class="comment-item">
                <img src="uploads/profile_pics/<?php echo e($comment['profile_pic'] ?? 'default.png'); ?>" class="comment-avatar">
                <div>
                    <strong><?php echo e($comment['name']); ?></strong> 
                    <small style="color:#95a5a6;"><?php echo date('M d, g:i a', strtotime($comment['created_at'])); ?></small>
                    <p style="margin-top:8px;"><?php echo e($comment['comment_text']); ?></p>
                </div>
            </div>
        <?php endforeach; ?>
    </div>
